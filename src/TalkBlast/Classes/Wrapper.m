...
- (void)sendRequestTo:(NSURL *)url usingVerb:(NSString *)verb withParameters:(NSDictionary *)parameters withBody:(NSString *)xml {
    NSData *body = nil;
    NSMutableString *params = nil;
    NSString *contentType = @"text/xml; charset=utf-8";
    NSURL *finalURL = url;
	
	// if there are parameters, start creating a string of name/value pairs
    if (nil != parameters) {
	...
	// if params have been created we append it correctly to either the body or the url
	if (nil != params) {
		if ([verb isEqualToString:HTTP_POST] || [verb isEqualToString:HTTP_PUT]) {
	...
	NSLog(@"Calling URL: %@ with method: %@", finalURL, verb);
	
	// if this is a pure XML content post the body parameter should be there
	if (nil != xml) {
