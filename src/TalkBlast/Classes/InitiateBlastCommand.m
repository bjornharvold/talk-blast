//
//  InitiateConferenceCallCommand.m
//  TalkBlast
//
...
@implementation InitiateBlastCommand
...
- (void) execute:(TalkBlastEvent *)event {
	if ([event class] == [InitiateBlastEvent class]) {
		InitiateBlastEvent *theEvent = (InitiateBlastEvent *) event;
...
/*!
 ...
 The data should be a CommunicationEvent object in XML form so we parse the data and retrieve a CommunicationEvent object. If 
...
- (void)wrapper:(Wrapper *)wrapper didRetrieveData:(NSData *)data {
	...
	NSArray *mediaContents = [parser parse:data error:&error];
	...
	if (nil == error && [caller respondsToSelector:@selector(initiateBlastResult:)]) {
		[caller initiateBlastResult:mediaContents];
	} else if ([caller respondsToSelector:@selector(initiateBlastFault:)]) {
		[caller initiateBlastFault:error];
	}
...
- (void)wrapperHasBadCredentials:(Wrapper *)wrapper {
	...
	if ([caller respondsToSelector:@selector(initiateBlastFault:)]) {
		[caller initiateBlastFault:error];
	}
}
