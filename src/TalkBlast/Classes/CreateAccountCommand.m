...
- (void) execute:(TalkBlastEvent *)event {
	...
	if ([event class] == [CreateAccountEvent class]) {
		...
		// send request
		[engine sendRequestTo:url usingVerb:HTTP_POST withParameters:nil withBody:xml];    
	} else {
...
- (void)wrapper:(Wrapper *)wrapper didRetrieveData:(NSData *)data {
	...
	if (nil == error && [caller respondsToSelector:@selector(createAccountResult:)]) {
		[caller createAccountResult:account];
	} else if ([caller respondsToSelector:@selector(createAccountFault:)]) {
		[caller createAccountFault:error];
	}
...
- (void)wrapperHasBadCredentials:(Wrapper *)wrapper {
	...
	if ([caller respondsToSelector:@selector(createAccountFault:)]) {
		[caller createAccountFault:error];
	}
}
